...
describe("orgInvitationRouter.acceptOrgInvitation", () => {
  ...
  it("accepts invitation and creates membership for new member", async () => {
    ...
    const txOrgMemberCreate = vi.fn().mockResolvedValue({});
    mockTransaction.mockImplementation(async (fn: (tx: unknown) => unknown) => {
      const tx = {
        organizationInvitation: { update: vi.fn() },
        organizationMember: { create: txOrgMemberCreate },
        user: { update: vi.fn() },
      };
    ...
    expect(txOrgMemberCreate).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ role: mockInvitation.role }),
      })
    );
  });
  ...
  it("accepts invitation and updates role for existing member (re-invite)", async () => {
    ...
    const txOrgMemberUpdate = vi.fn().mockResolvedValue({});
    mockTransaction.mockImplementation(async (fn: (tx: unknown) => unknown) => {
      const tx = {
        organizationInvitation: { update: vi.fn() },
        organizationMember: { update: txOrgMemberUpdate },
        user: { update: vi.fn() },
      };
      const { applyWorkspaceAssignments } =
        await import("@/core/org-invitation-accept");
    ...
    expect(txOrgMemberUpdate).toHaveBeenCalledWith(
      expect.objectContaining({
