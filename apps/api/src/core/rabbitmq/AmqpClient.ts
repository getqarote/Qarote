...
export class RabbitMQAmqpClient {
  ...
  /**
   * Create a dedicated channel for isolated operations (e.g., spy sessions).
   * Unlike the shared channel, a dedicated channel isolates failures —
   * a channel error won't tear down other consumers on the main channel.
   * The caller owns the channel lifecycle and must close it when done.
   */
  async createDedicatedChannel(): Promise<amqp.Channel> {
    await this.ensureConnection();
    ...
    logger.info(
      ...
      "Created dedicated AMQP channel"
    );
  ...
  /**
   * Bind a queue to an exchange on a given channel.
   * Used for spy sessions to replicate target queue bindings onto a temporary queue.
   */
  async bindQueue(
    ...
    try {
      ...
      logger.debug({ queue, exchange, routingKey }, "Bound queue to exchange");
    } catch (error) {
      logger.error(
        { error, queue, exchange, routingKey },
        "Failed to bind queue to exchange"
      );

      if (error instanceof Error) {
        captureRabbitMQError(error, {
          operation: "amqp_bind_queue",
          queueName: queue,
        });
      }
  ...
  /**
   * Delete a queue on a given channel.
   * Used for cleanup of temporary spy queues.
   */
  async deleteQueue(channel: amqp.Channel, queue: string): Promise<void> {
    try {
      await channel.deleteQueue(queue);
      logger.debug({ queue }, "Deleted queue");
    } catch (error) {
      logger.error({ error, queue }, "Failed to delete queue");

      if (error instanceof Error) {
        captureRabbitMQError(error, {
          operation: "amqp_delete_queue",
          queueName: queue,
        });
      }
