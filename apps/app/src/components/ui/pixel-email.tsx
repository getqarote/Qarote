import { cn } from "@/lib/utils";

export function PixelEmail({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 477 371"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      shapeRendering="crispEdges"
      className={cn("w-auto", className)}
      aria-hidden="true"
    >
      <path d="M159 318H212V371H159V318Z" fill="currentColor" />
      <path d="M212 318H265V371H212V318Z" fill="currentColor" />
      <path d="M318 318H371V371H318V318Z" fill="currentColor" />
      <path d="M371 318H424V371H371V318Z" fill="currentColor" />
      <path d="M265 318H318V371H265V318Z" fill="currentColor" />
      <path d="M424 212H477V265H424V212Z" fill="currentColor" />
      <path d="M424 159H477V212H424V159Z" fill="currentColor" />
      <path d="M424 106H477V159H424V106Z" fill="currentColor" />
      <path d="M0 106H53V159H0V106Z" fill="currentColor" />
      <path d="M318 0H371V53H318V0Z" fill="currentColor" />
      <path d="M265 0H318V53H265V0Z" fill="currentColor" />
      <path d="M212 0H265V53H212V0Z" fill="currentColor" />
      <path d="M159 0H212V53H159V0Z" fill="currentColor" />
      <path d="M0 159H53V212H0V159Z" fill="currentColor" />
      <path d="M106 0H159V53H106V0Z" fill="currentColor" />
      <path d="M53 53H106V106H53V53Z" fill="currentColor" />
      <path d="M0 53H53V106H0V53Z" fill="currentColor" />
      <path d="M53 0H106V53H53V0Z" fill="currentColor" />
      <path d="M424 53H477V106H424V53Z" fill="currentColor" />
      <path d="M371 53H424V106H371V53Z" fill="currentColor" />
      <path d="M371 0H424V53H371V0Z" fill="currentColor" />
      <path d="M106 106H159V159H106V106Z" fill="currentColor" />
      <path d="M159 159H212V212H159V159Z" fill="currentColor" />
      <path d="M212 212H265V265H212V212Z" fill="currentColor" />
      <path d="M265 159H318V212H265V159Z" fill="currentColor" />
      <path d="M318 106H371V159H318V106Z" fill="currentColor" />
      <path d="M0 212H53V265H0V212Z" fill="currentColor" />
      <path d="M0 265H53V318H0V265Z" fill="currentColor" />
      <path d="M424 265H477V318H424V265Z" fill="currentColor" />
      <path d="M106 318H159V371H106V318Z" fill="currentColor" />
      <path d="M53 318H106V371H53V318Z" fill="currentColor" />
    </svg>
  );
}
