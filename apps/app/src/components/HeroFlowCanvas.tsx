/* ============================================================================
 * HeroFlowCanvas — the animated message-flow graph (ported from the landing's
 * apps/web HeroFlowCanvas, which is itself ported from
 * docs/reference/HeroFlowCanvas.reference.tsx). Used here behind the auth
 * panel so sign-in/up shows the same "agent scans the broken queue" story.
 * ----------------------------------------------------------------------------
 ...
 * prefers-reduced-motion (static end-frame), pauses when off-screen (observes
 * the canvas itself), simplifies to 3 nodes < 720px. Decorative → aria-hidden.
 * Colors hardcoded (fixed near-black panel that doesn't theme); carrot #E8590C
 * matches the token.
 * ==========================================================================*/
...
export default function HeroFlowCanvas() {
  ...
  useEffect(() => {
    ...
    function layout() {
      ...
      const pubX = W * (smallScreen ? 0.16 : 0.28);
      const exX = W * (smallScreen ? 0.42 : 0.48);
      const qX = W * (smallScreen ? 0.8 : 0.7);
      const cy = H * 0.5;
    ...
    if (reduce) {
      ...
      // Observe the canvas itself (works regardless of the wrapping element)
      // so we pause when the panel scrolls off-screen.
