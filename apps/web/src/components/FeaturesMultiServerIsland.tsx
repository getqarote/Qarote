...
const NS = "features-multi-server";
...
function TeamWorkspacesSection() {
  const { t } = useTranslation(NS);

  const paragraphs: string[] = [];
  for (const key of ["p1", "p2", "p3"]) {
    const val = t(`teamWorkspaces.${key}`, { defaultValue: "" });
...
export function FeaturesMultiServerIsland({
  locale = "en",
  resources,
}: IslandProps) {
  return (
