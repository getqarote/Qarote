...
const NS = "features-message-tracing";
...
function TraceTypesSection() {
  const { t } = useTranslation(NS);
  const reduceMotion = useReducedMotion();
  const [gridRef, gridEntered] = useScrollEntry<HTMLDivElement>(0.05);

  const items = ((): TraceTypeItem[] => {
    const raw = t("traceTypes.items", { returnObjects: true });
    return Array.isArray(raw) ? (raw as TraceTypeItem[]) : [];
  })();
...
function WhyTracingSection() {
  const { t } = useTranslation(NS);

  const paragraphs: string[] = [];
  for (const key of ["p1", "p2", "p3", "p4"]) {
    const val = t(`whyTracing.${key}`, { defaultValue: "" });
...
export function FeaturesMessageTracingIsland({
  locale = "en",
  resources,
}: IslandProps) {
  return (
