...
const NS = "features-queue-management";
...
function OperationsSection() {
  ...
  const items = ((): OperationItem[] => {
    const raw = t("operations.items", { returnObjects: true });
    return Array.isArray(raw) ? (raw as OperationItem[]) : [];
  })();
...
function WhyCLISection() {
  const { t } = useTranslation(NS);

  const paragraphs: string[] = [];
  for (const key of ["p1", "p2", "p3"]) {
    const val = t(`whyCLI.${key}`, { defaultValue: "" });
...
export function FeaturesQueueManagementIsland({
  locale = "en",
  resources,
}: IslandProps) {
  return (
