...
const NS = "features-incident-diagnosis";
...
function DiagnosisTypesSection() {
  const { t } = useTranslation(NS);
  const reduceMotion = useReducedMotion();
  const [gridRef, gridEntered] = useScrollEntry<HTMLDivElement>(0.05);

  const items = ((): DiagnosisTypeItem[] => {
    const raw = t("diagnosisTypes.items", { returnObjects: true });
    return Array.isArray(raw) ? (raw as DiagnosisTypeItem[]) : [];
  })();
...
function WhyDiagnosisSection() {
  const { t } = useTranslation(NS);

  const paragraphs: string[] = [];
  for (const key of ["p1", "p2", "p3", "p4"]) {
    const val = t(`whyDiagnosis.${key}`, { defaultValue: "" });
...
export function FeaturesIncidentDiagnosisIsland({
  locale = "en",
  resources,
}: IslandProps) {
  return (
