...
function MetricTypesSection() {
  ...
  const items = ((): MetricTypeItem[] => {
    const raw = t("metricTypes.items", { returnObjects: true });
    return Array.isArray(raw) ? (raw as MetricTypeItem[]) : [];
  })();
...
function WhyPersistenceSection() {
  const { t } = useTranslation(NS);

  const paragraphs: string[] = [];
  for (const key of ["p1", "p2", "p3", "p4"]) {
    const val = t(`whyPersistence.${key}`, { defaultValue: "" });
...
export function FeaturesMetricsIsland({
  locale = "en",
  resources,
}: IslandProps) {
  return (
