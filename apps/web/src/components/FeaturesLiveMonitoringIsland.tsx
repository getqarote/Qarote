...
const NS = "features-live-monitoring";
...
// ---------------------------------------------------------------------------
// Why not management plugin
// ---------------------------------------------------------------------------

function WhyManagementPluginSection() {
  const { t } = useTranslation(NS);

  const paragraphs: string[] = [];
  for (const key of ["p1", "p2", "p3", "p4"]) {
    const val = t(`whyManagementPlugin.${key}`, { defaultValue: "" });
...
export function FeaturesLiveMonitoringIsland({
  locale = "en",
  resources,
}: IslandProps) {
  return (
