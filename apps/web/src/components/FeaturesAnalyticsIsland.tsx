...
function ChartTypesSection() {
  const { t } = useTranslation(NS);
  const reduceMotion = useReducedMotion();
  const [gridRef, gridEntered] = useScrollEntry<HTMLDivElement>(0.05);

  const items = ((): ChartTypeItem[] => {
    const raw = t("chartTypes.items", { returnObjects: true });
    return Array.isArray(raw) ? (raw as ChartTypeItem[]) : [];
  })();
...
function VsPrometheusSection() {
  const { t } = useTranslation(NS);

  const paragraphs: string[] = [];
  for (const key of ["p1", "p2", "p3"]) {
    const val = t(`vsPrometheus.${key}`, { defaultValue: "" });
    ...
    <div className="border border-border overflow-hidden mb-8">
      ...
      <nav aria-label={t("crossLink.navLabel")}>
        <ul className="flex flex-wrap gap-x-6 gap-y-3 items-center list-none p-6">
          ...
          <li>
            <a
              href={lp("/features/live-monitoring/")}
              className="text-sm text-muted-foreground hover:text-foreground transition-colors"
            >
              {t("crossLink.liveMonitoring")}
            </a>
          </li>
          <li aria-hidden="true">
            <span className="text-muted-foreground text-xs">·</span>
...
export function FeaturesAnalyticsIsland({
  locale = "en",
  resources,
}: IslandProps) {
  return (
