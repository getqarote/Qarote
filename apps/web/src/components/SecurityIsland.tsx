        ...
        <ol className="grid sm:grid-cols-2 gap-x-8 gap-y-2 text-sm list-none pl-0">
          {sections.map((s, i) => (
            <li key={s.id}>
              ...
              >
                <span className="text-xs text-primary/50 shrink-0 transition-colors duration-150 group-hover:text-primary font-mono">
...
function Pillars() {
  ...
  const itemsRaw = t("pillars.items", { returnObjects: true });
  const items: PillarItem[] = Array.isArray(itemsRaw)
    ? (itemsRaw as PillarItem[])
    : [];
    ...
    <div className="border border-border overflow-hidden mb-8">
      <div className="px-6 py-3 bg-muted/30 border-b border-border flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground/50 font-mono">
          {t("pillars.controlsCount", { count: items.length })}
        </span>
      </div>
        ...
        {items.map((item, i) => (
          ...
          >
            <div className="flex items-start gap-3 sm:block">
              <span className="text-xs text-primary/50 leading-none mt-0.5 sm:mb-2 shrink-0 transition-colors duration-200 group-hover:text-primary/80 font-mono">
                {String(i + 1).padStart(2, "0")}
              </span>
        ...
        {items.map((item, i) => (
          <div key={i} style={itemStyle(i)}>
            ...
            >
              ...
              <svg
                className={`w-4 h-4 shrink-0 text-muted-foreground mt-0.5 transition-transform duration-200 ${open === i ? "rotate-180" : ""}`}
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth={2}
