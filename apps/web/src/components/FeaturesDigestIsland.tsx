...
function DigestSectionsSection() {
  const { t } = useTranslation(NS);
  const reduceMotion = useReducedMotion();
  const [gridRef, gridEntered] = useScrollEntry<HTMLDivElement>(0.05);

  const items = ((): DigestSectionItem[] => {
    const raw = t("digestSections.items", { returnObjects: true });
    return Array.isArray(raw) ? (raw as DigestSectionItem[]) : [];
  })();
...
function WhyDigestSection() {
  const { t } = useTranslation(NS);

  const paragraphs: string[] = [];
  for (const key of ["p1", "p2", "p3", "p4"]) {
    const val = t(`whyDigest.${key}`, { defaultValue: "" });
...
export function FeaturesDigestIsland({
  locale = "en",
  resources,
}: IslandProps) {
  return (
