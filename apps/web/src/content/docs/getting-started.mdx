...
Used for features that need a live connection to the broker. Qarote opens a direct AMQP connection and closes it when the feature is no longer in use.
...
<Callout type="tip" title="Community Edition is free">
  Qarote's core monitoring features are free forever under the MIT license. No
  credit card required. Premium features (workspaces, alerting, SSO) are
  unlocked via a license key.
</Callout>
