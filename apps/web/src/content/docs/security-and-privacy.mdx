...
- **[Tracing](/docs/tracing-vs-spy/)** captures broker metadata for every published and delivered message — routing key, exchange, queue, content-type, payload size, and timestamp — and persists it in your PostgreSQL database for the configured retention window (7 days by default). The body itself is never read or stored.
