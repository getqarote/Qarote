...
| Metrics snapshots                     | For trend charts (fixed 30-day retention) |

Qarote does **not** store message bodies. Two features touch message-level data, and they handle it differently:

- **[Tracing](/docs/tracing-vs-spy/)** captures broker metadata for every published and delivered message — routing key, exchange, queue, content-type, payload size, and timestamp — and persists it in your PostgreSQL database for a fixed 7-day retention window. The body itself is never read or stored.
- **Spy** displays full message bodies live in your browser only. They are never written to disk. When the spy session ends, the captured data is gone.
