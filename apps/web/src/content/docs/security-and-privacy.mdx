...
- **Tracing** captures broker metadata for every published and delivered message — routing key, exchange, queue, content-type, payload size, and timestamp — and persists it in your PostgreSQL database for a fixed 7-day retention window. The body itself is never read or stored.
