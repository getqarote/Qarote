...
> **无需搭建Prometheus。** Qarote直接连接RabbitMQ的Management API，内置上述五个信号的告警规则——带变化率的queue深度、未确认消息数、consumer降至零（立即呼人）、内存watermark百分比，以及DLQ增长。在UI中按queue配置阈值，并设置通知目标（Slack、webhook）。[了解告警功能 →](/zh/features/alerting/)
